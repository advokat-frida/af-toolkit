...
const samples = JSON.stringify({
  send: { name: 'cadence-audience.csv', text: read('samples/cadence-audience.csv') },
...
const netKill = `(function(){
  ...
  window.__safelistNetViolations=function(){return netCount;};`;

function page({ bodyHtml, extraCss = '' }) {
  return `<!DOCTYPE html>
<!--
  Advokat Frida: SafeList
  Your send list, minus everyone who said no. Everything runs in this page;
  nothing is fetched, stored, or sent. The record names what was checked.
-->
<html lang="en">
<head>
<meta charset="utf-8">
...
writeFileSync(root('dist/safelist.html'), standalone);
console.log(`dist/safelist.html ${kb(standalone)} (gzip ${kb(gzipSync(Buffer.from(standalone)))})`);

const embed = page({ bodyHtml: embedBody, extraCss: embedCss });
writeFileSync(root('dist/safelist-embed.html'), embed);
console.log(`dist/safelist-embed.html ${kb(embed)} (gzip ${kb(gzipSync(Buffer.from(embed)))})`);
