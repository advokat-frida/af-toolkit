...
- `0.4.1` is committed on `main` here. It is released by publishing a stable GitHub Release on
  this repository tagged `safeseed-v0.4.1`. (The immutable `safeseed-v0.4.0` Release exists but
  never published: its publish job hit the tarball-path bug that 0.4.1 fixes.)
...
- `package.json`, `package-lock.json`, `SAFESEED_VERSION`, the example run record, README Action tag,
  and top changelog entry all say `0.4.1`.
...
1. Draft a GitHub Release titled `SafeSeed 0.4.1`, targeting the exact reviewed commit on `main`
   and tag `safeseed-v0.4.1`.
2. Use the `0.4.1` changelog entry as release notes. Attach every intended asset before
...
```powershell
npm.cmd view safeseed@0.4.1 version dist.integrity dist.shasum --json
npx.cmd --yes safeseed@0.4.1 version
```

The expected CLI line begins `safeseed 0.4.1`. Confirm npm shows provenance and that the package,
